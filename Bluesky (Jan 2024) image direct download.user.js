...
// @grant        GM_addStyle
// @run-at       document-end
// @downloadURL  https://github.com/StelardActek/UserScripts/raw/refs/heads/master/Bluesky%20(Jan%202024)%20image%20direct%20download.user.js
...
(async function() {
  ...
  let addTags = () => {
    ...
    let containers = document.querySelectorAll("div[data-expoimage=true]:has(img[src*=feed_thumbnail]):not(.has-dlanchor)");
    if (containers && containers.length) {
      ...
      for (let container of containers) {
        ...
        if (img) {
          ...
          let article = container.closest("div[role=link][data-testid]");
          if (article) {
            console.log('found article', article);
            let alink = article.querySelector('a.css-146c3p1.r-1loqt21[role=link][href^="/profile"][href*="/post/"]');
            if (alink && alink.href) {
              let postMatch = alink.href.match(postRegex);
